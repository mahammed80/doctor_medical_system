...
const services = [
  {
    ...
    price: '٨٩٩',
    period: 'ريال',
    desc: 'جلسة استشارية شاملة لمدة ٣٠ دقيقة مع الطبيب لتقييم حالتك وتشخيصها بدقة.',
    features: [
      'كشفية أساسية مع الاستشاري',
      'عرض وتحليل الأشعة والتحاليل',
      'تشخيص طبي دقيق للحالة',
      'مدة الجلسة: ٣٠ دقيقة',
      'متابعة مجانية لمدة ١٠ أيام',
    ],
    popular: false,
  },
  {
  ...
  },
  {
    ...
    features: [
      'باقة ٣ جلسات — بقيمة ٢٬٥٠٠ ريال',
      'باقة ٤ جلسات — بقيمة ٣٬٤٠٠ ريال',
      'توفير يصل إلى ٢٠٠ ريال',
...
const faqs = [
  { q: 'كيف يمكنني حجز استشارة في مركز بترجي؟', a: 'العملية بسيطة للغاية وتستغرق أقل من ٥ دقائق: اختر التخصص والطبيب المناسب لحالتك، املأ بياناتك الطبية وأرفق الأشعة إن وجدت، قم بسداد رسوم الاستشارة بأمان، ثم اختر الموعد المناسب لك من جدول الطبيب المباشر لتلقي رابط الجلسة المرئية.' },
  { q: 'ما هي تكلفة الاستشارة الطبية؟', a: 'تختلف التكلفة حسب مستوى الكشف المختار: الكشف الأساسي بقيمة ٨٩٩ ريال، الكشف الشامل وإعداد الخطة العلاجية بقيمة ١,٧٠٠ ريال، كما تتوفر باقات للمتابعات المتعددة تبدأ من ٢,٥٠٠ ريال لـ ٣ جلسات.' },
  { q: 'هل يمكنني مراجعة الطبيب مجاناً بعد الجلسة؟', a: 'نعم، تشمل جميع الاستشارات الطبية كفترة مراجعة (متابعة مجانية) صالحة لمدة ١٠ أيام من تاريخ الجلسة الأساسية لمناقشة نتائج التحاليل أو تحديث خطة العلاج.' },
...
const qualifications = [
  { year: '١٩٩١', title: 'تأسيس مركز بترجي الطبي', inst: 'رؤية لتقديم أفضل الخدمات الطبية التخصصية' },
  { year: '٢٠١٢', title: 'إطلاق العيادات المتكاملة', inst: 'توسعة الأقسام لتشمل العمود الفقري والروماتيزم والتأهيل' },
  { year: '٢٠٢٠', title: 'التكامل الرقمي الكامل', inst: 'تحويل جميع الاستشارات إلى استشارات مرنية تفاعلية عن بعد' },
  { year: '٢٠٢٦', title: 'نظام الاستشارات المطور', inst: 'إطلاق المنصة الجديدة وتفعيل ميزات التخزين الطبي الآمن وتوصيل الأدوية' },
]

const resources = [
  { title: 'دليل المريض الشامل لجراحات المفاصل والمنظار', summary: 'كل ما تحتاج معرفته عن كيفية التحضير لعملية المنظار وما يمكن توقعه في مرحلة التعافي.', tag: 'جراحة العظام', readTime: '٥ دقائق' },
  { title: 'طرق التمييز بين آلام العضلات والروماتيزم', summary: 'دليل مبسط لمساعدتك في فهم طبيعة الألم ومتى يتعين عليك استشارة طبيب روماتيزم متخصص.', tag: 'الروماتيزم', readTime: '٣ دقائق' },
  { title: 'تمارين وقائية لتقوية أسفل الظهر وحماية العمود الفقري', summary: 'مجموعة من التمارين اليومية البسيطة التي تقي من الانزلاق الغضروفي وتخفف آلام الجلوس الطويل.', tag: 'العمود الفقري', readTime: '٤ دقائق' },
]
...
function AnimatedCounter({ end, suffix = '' }: { end: number; suffix?: string }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const arabicMap: Record<string, string> = { '0': '٠', '1': '١', '2': '٢', '3': '٣', '4': '٤', '5': '٥', '6': '٦', '7': '٧', '8': '٨', '9': '٩' }
      el.textContent = numStr.replace(/\d/g, d => arabicMap[d] || d) + suffix
    }, 25)
    return () => clearInterval(interval)
  }, [started, end, suffix])

  return <span ref={ref}>٠{suffix}</span>
}
                ...
                {stats.map(({ num, label, desc }) => (
                  <div key={label} style={{ position: 'relative' }}>
                    <div className="num" style={{
                      ...
                      fontWeight: 900,
                      letterSpacing: '-0.03em',
                      background: 'linear-gradient(135deg, var(--primary) 0%, oklch(46% 0.19 260 / 0.7) 100%)',
                      WebkitBackgroundClip: 'text',
                      WebkitTextFillColor: 'transparent',
                      backgroundClip: 'text',
                      lineHeight: 1.1,
                    ...
                    <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--fg)', marginTop: '0.1rem' }}>{label}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--fg-dim)', marginTop: '0.05rem' }}>{desc}</div>
                  </div>
                ))}
              </div>
            </div>
